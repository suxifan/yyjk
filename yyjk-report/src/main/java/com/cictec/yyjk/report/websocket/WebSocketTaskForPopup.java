...
@Component
public class WebSocketTaskForPopup extends BaseController {
	private static final Logger LOG = LoggerFactory.getLogger(WebSocketTaskForPopup.class);
	@Autowired
	...
	@Scheduled(cron = "${jobs.push.warn.opup.datas}")
	public void start() throws Exception {
		// 推送数据
		Map<String, WebSocketServletForPopup> webSockets = WebSocketServletForPopup.getWebSocketMap();
		LOG.info("疲劳报警弹出框推送数据开始！");
		if (webSockets != null && webSockets.size() > 0) {
			for (String key : webSockets.keySet()) {
				WebSocketServletForPopup webSocket = webSockets.get(key);
